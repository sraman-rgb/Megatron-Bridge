...
class ConfigContainer(Container):
    ...
    def validate(self) -> None:
        ...
        if self.dist.use_megatron_fsdp or self.ddp.use_megatron_fsdp:
            ...
            # reuse_grad_buf_for_high_precision_param_ag is not supported with Megatron FSDP
            if self.ddp.reuse_grad_buf_for_high_precision_param_ag:
                print_rank_0("reuse_grad_buf_for_high_precision_param_ag is not supported with Megatron FSDP, setting to False")
                self.ddp.reuse_grad_buf_for_high_precision_param_ag = False
            if self.optimizer.reuse_grad_buf_for_high_precision_param_ag:
                self.optimizer.reuse_grad_buf_for_high_precision_param_ag = False
